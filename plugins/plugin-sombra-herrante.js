...
const SombraHerrante = (() => {
  ...
  function _battleSombra(nodeId, sombra) {
    ...
    if(!sombra || (typeof getCurrentBattle === 'function' ? getCurrentBattle() : Net.getMyBattle?.())) return;
    const p = Player.get();
    ...
    else Net.startBattle(nodeId, actors);
  }
  ...
  function _wander(payload) {
    ...
    const getCurrentBattle = ServiceRegistry?.get?.('gameplay.battle.current');
    if(!found || (typeof getCurrentBattle === 'function' ? getCurrentBattle() : Net.getMyBattle?.())) return payload;
