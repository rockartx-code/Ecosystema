// ════════════════════════════════════════════════════════════════
// PLUGIN: Tacticas v2.0
// Combate tactico y descanso movidos a plugin de dominio.
// ════════════════════════════════════════════════════════════════
...
(function initTacticsPlugin(global) {
  ...
  function loadTacticsData() {
    ...
    try { return ModuleLoader?.getSystemData?.('tactics', fallback) || fallback; }
    catch {}
    return fallback;
  }

  function createTacticsApi() {
    const data = loadTacticsData();
    ...
    function aplicarElemento(combatiente, elemento) {
    ...
    function calcularDaño(actor, target, arma, elemento, battle) {
      ...
      if(elemento && target.elemento_estado) {
        const k = `${target.elemento_estado}+${elemento}`;
        reaccion = REACCIONES[k];
        if(reaccion) reacMult = reaccion.mult * climaMult;
      }
  ...
  function api() {
    if(!global.__tacticsPluginApi) global.__tacticsPluginApi = createTacticsApi();
    return global.__tacticsPluginApi;
  }

  global.pluginTacticas = {
    id: 'plugin:tacticas',
    nombre: 'Sistema Tactico',
    version: '2.0.0',
    descripcion: 'Estado elemental, superficies, poise y descanso cargados como plugin.',
    onLoad() { api(); },
    services: {
      'runtime.tactics.api': () => api(),
      'runtime.player.rest': async () => api().cmdDescansar(),
      'runtime.player.tactic': () => api().cmdTactica(),
      'runtime.tactics.consume_stamina': (delta=0) => { api().consumirStamina(delta); return true; },
      'runtime.tactics.calc_wound': (dmg=0, maxHp=1) => api().calcularHerida(dmg, maxHp),
      'runtime.tactics.wound_meta': (key='') => api().HERIDAS?.[key] || null,
      'runtime.tactics.apply_element': (target, element, battle=null) => { api().aplicarElemento(target, element, battle); return true; },
      ...
      'runtime.tactics.update_surface': (nodeId, element, battle=null) => { api().actualizarSuperficie(nodeId, element, battle); return true; },
      'runtime.tactics.element_color': (element='') => api().ELEMENTOS?.[element]?.color || null,
      'runtime.tactics.apply_poise_dmg': (target, amount=0, battle=null) => { api().aplicarPoiseDmg(target, amount, battle); return true; },
    },
  };
})(globalThis);
