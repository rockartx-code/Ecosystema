// ════════════════════════════════════════════════════════════════
// NPC ENGINE — Generación, diálogo, misiones y consecuencias
// ════════════════════════════════════════════════════════════════
const NPCEngine = {
  ...
  nombre(rng) {
    ...
    return U.chance(nd.titulo_chance||.3) ? n + ' ' + U.pick(nd.nombres_titulos||[''],rng) : n;
  },
  ...
  gen(nodeId, wseed, idx, ecos, otros) {
    ...
    const nd  = D.npcs;
    let npc = {
      id: U.uid(), nombre: this.nombre(rng),
      arq_vis: U.pick(nd.arquetipos_visibles||['errante'],rng),
      arq_ocu: U.pick(nd.arquetipos_ocultos||['vacío'],rng),
      deseo:   U.pick(nd.deseos||['—'],rng),
      necesidad: U.pick(nd.necesidades||['—'],rng),
      miedo:   U.pick(nd.miedos||['—'],rng),
      secreto: U.pick(nd.secretos||['—'],rng),
      secreto_idx: Math.floor(rng()*(nd.secretos||['—']).length),
      trauma:  U.pick(nd.traumas||['—'],rng),
      lealtad: U.rand(10,40) + (Player.get().presencia||0)*3,
      corrupcion: U.rand(5,30),
      desesperacion: U.rand(10,50),
      nodeId, estado:'vivo',
      secreto_expuesto:false, arq_ocu_expuesto:false,
      fragmentos:[], vinculos:[], misiones_ofrecidas:[],
      interacciones:0,
      eco_run_id: ecos?.length && U.chance(.3) ? U.pick(ecos,rng).run_id : null,
      twists_activados:[], hp_combat:null, faccion:null,
    };
    if(otros?.length && U.chance(nd.vinculo_chance||.6)) {
  ...
  dialogo(npc, runs) {
    ...
    let pool = est === 'normal'
      ? (dl.saludo?.[npc.arq_vis] || [])
      : (dl.estados?.[est] || dl.saludo?.[npc.arq_vis] || []);
    if(!pool.length) pool = [`${npc.nombre} te mira en silencio.`];
    let text = U.pick(pool,rng).replace(/{n}/g,npc.nombre);

    if(npc.lealtad > 50 && U.chance(.4) && !npc.fragmentos.includes('deseo')) {
      text += ` "${npc.deseo}."`;
      npc.fragmentos.push('deseo');
    }
    if(npc.eco_run_id) {
      const er = runs?.find(r=>r.id===npc.eco_run_id);
      if(er && U.chance(.45)) text += ` "Hubo alguien antes de ti. ${er.player_name||'Sin nombre'}."`;
    }
  ...
  interactuar(npc) {
    ...
    if(npc.lealtad > 60 && !npc.secreto_expuesto && U.chance(.25)) {
      npc.secreto_expuesto = true;
      if(typeof XP !== 'undefined') XP.ganar('narrativa', 25, 'secreto descubierto');
      return { tipo:'secreto', texto:`${npc.nombre}: "${npc.secreto}"` };
    }
    if(npc.lealtad > 80 && !npc.arq_ocu_expuesto && U.chance(.2)) {
      npc.arq_ocu_expuesto = true;
      if(typeof XP !== 'undefined') XP.ganar('narrativa', 40, 'naturaleza oculta revelada');
      return { tipo:'arquetipo', texto:`${npc.nombre} deja de actuar. Es ${npc.arq_ocu}. Siempre lo fue.` };
    }
    return null;
  },
  ...
  observar(npc) {
    const obs = [
      `${npc.nombre} — ${npc.arq_vis.toUpperCase()}`,
      `Lealtad:${npc.lealtad}  Corrupción:${npc.corrupcion}  Desesperación:${npc.desesperacion}`,
    ];
  ...
  combatLine(poolKey, npc) {
    const rng  = U.rng(npc.id + Math.random());
    const dl   = D.npcs.dialogos || {};
    let pool   = dl[poolKey]?.normal || dl[poolKey] || [];
    if(npc.desesperacion > 75 && dl[poolKey]?.desesperado?.length) pool = dl[poolKey].desesperado;
    if(!Array.isArray(pool) || !pool.length) return `${npc.nombre}.`;
    return U.pick(pool,rng).replace(/{n}/g,npc.nombre).replace(/\{deseo\}/g,npc.deseo||'—');
  },

  checkTwists(npc, player, misiones, ecos, wd) {
    const act = [];
  ...
  genMision(npc, npcs, ecos) {
    ...
    const vars = {
      npc: npc.nombre, victima: vic?.nombre||'alguien',
      eco: eco?.nombre||'alguien',
      objetivo: tpl.objetivos?.length ? U.pick(tpl.objetivos,rng) : 'algo',
      costo:    tpl.costos?.length    ? U.pick(tpl.costos,rng)    : 'algo',
      amenaza:  tpl.amenazas?.length  ? U.pick(tpl.amenazas,rng)  : 'algo',
      verdad:   tpl.verdades?.length  ? U.pick(tpl.verdades,rng)  : 'algo',
      ciclos:   U.rand(...(tpl.ciclos_rng||[2,10])),
    };

    let mision = {
      id: U.uid(), tipo, npc_id: npc.id,
      titulo: U.tmpl(tpl.titulo||tipo, vars),
      desc:   U.tmpl(tpl.desc||'—', vars),
      objetivo: vars.objetivo,
      recompensa: tpl.recompensas?.length ? U.pick(tpl.recompensas,rng) : 'algo',
      consecuencia_fallo: U.tmpl(tpl.consecuencia||'—', vars),
      victima_id: vic?.id || null,
      eco_id:     eco?.run_id || null,
      es_trampa:  tipo==='traición' && U.chance(tpl.trampa_chance||0),
      es_imposible: tipo==='imposible',
      completada:false, fallida:false, aceptada:false,
      ciclo: Clock.cycle,
    };
  ...
  narrativaLoot(npc, pool) {
    const source = pool==='rendicion' ? (D.narrative.loot_rendicion||{}) : (D.narrative.loot_narrativo||{});
    const items  = [];
    for(const [bp, def] of Object.entries(source)) {
      if(U.chance(def.chance||1)) {
        const vars = { npc:npc.nombre, deseo:npc.deseo, secreto:npc.secreto, trauma:npc.trauma };
        items.push({ id:U.uid(), blueprint:bp, nombre:U.tmpl(def.nombre,vars), tipo:'reliquia', tags:['narrativo','antiguo'], estado:'nativo', desc:U.tmpl(def.desc,vars), npc_origen:npc.id });
      }
    }
    return items;
  },
  ...
  consecuenciaDesperación(npc) {
    ...
    } else if(r < (nd.muerte_chance||.3) + (nd.desaparicion_chance||.3)) {
  ...
  // Tick de NPCs vivos (desesperación, corrupción)
  tickNPCs() {
    const nd  = D.npcs || {};
    const cfg = D;
    GS.aliveNPCs().forEach(npc => {
      if(npc.estado !== 'vivo') return;
      if(U.chance(cfg.npc_desesperacion_tick_chance||.12) && npc.desesperacion < (cfg.npc_desesperacion_tick_max||5)*20) {
        npc.desesperacion = Math.min(100, npc.desesperacion + U.rand(3,8));
        if(npc.desesperacion >= 90 && U.chance(cfg.npc_quiebre_chance||.15)) {
          this.consecuenciaDesperación(npc);
        }
      }
      if(U.chance(cfg.npc_corrupcion_tick_chance||.07) && npc.corrupcion < (cfg.npc_corrupcion_tick_max||3)*35) {
        npc.corrupcion = Math.min(100, npc.corrupcion + U.rand(2,6));
      }
    });
  },
};
